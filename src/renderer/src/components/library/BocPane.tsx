...
export function BocPane({
  ...
  const documentCode = pane.documentCode ?? 'AC'
  const sectionOrdinal = pane.sectionOrdinal ?? 1
  const bocSourceId = pane.bocSourceId ?? sources[0]?.id ?? ''
  ...
  const navigate = (code: string, ordinal: number): void => {
    setPaneContent(pane.id, { kind: 'boc', documentCode: code, sectionOrdinal: ordinal, bocSourceId })
    void api.setSession('lastBoc', JSON.stringify({ documentCode: code, ordinal }))
  }

  const pickSource = (id: string): void => {
    setPaneContent(pane.id, { kind: 'boc', documentCode, sectionOrdinal, bocSourceId: id })
  }
