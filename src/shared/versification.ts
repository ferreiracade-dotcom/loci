// Static per-chapter verse counts, keyed by USFM book code (matching scriptureRef.ts's
// BOOKS list) — index i holds chapter (i+1)'s verse count. Used only as a soft bounds
// check during commentary ingestion (Phase 3): a chunk whose verse number exceeds its
// chapter's count is flagged for review, not silently trusted or rejected outright.
//
// Sourced from the Berean Standard Bible via the Free Use Bible API
// (https://bible.helloao.org/api/BSB/{book}/{chapter}.json), the same provider Loci's own
// scripture.ts already uses for BSB — chosen over a generic "KJV baseline" so this table
// matches what the app actually displays. Versification varies by a verse or two between
// translations/editions in a handful of well-known spots (e.g. 3 John's final verses); this
// is expected and is exactly why bounds failures only flag a chunk for manual review rather
// than being treated as ground truth.

export type VersificationTable = Record<string, number[]>

export const VERSE_COUNTS: VersificationTable = {
